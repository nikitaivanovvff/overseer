...
The accept loop also guards the socket node itself. Unix keeps an already-bound listener and existing attach streams alive if the filesystem node is unlinked, but every new hook/CLI connection then fails with `ENOENT`; the daemon detects that condition and rebinds the same path without disturbing its registry, PTYs, or attached TUIs. This prevents a resolved permission prompt from leaving an agent permanently `blocked` merely because its clearing status push could no longer connect.
