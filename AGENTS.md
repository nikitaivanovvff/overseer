...
**A workspace's repo/name gets the identical live top-up, gated to root only.** The usual workflow — `overseer start` from a parent directory (e.g. `~/projects`, not itself a repo) and `cd` into the actual repo before launching an agent — used to leave every workspace named after that static launch cwd forever, since `name`/`repo` were set once at registration and never revisited. Every `overseer status` push now also self-reports the pushing process's current repo (`cli::detect_current_repo`: the git repo root's basename via `git rev-parse --show-toplevel`, same as `GitClient::repo_name`, falling back to the bare directory's own basename outside git, via `dir_basename` — never `None`, unlike branch), and the registry merges it into both `repo` and `name` — but **only when the pushing agent's `$OVERSEER_ROLE` is `root`** (checked CLI-side, so a child's hook fires don't pay for the extra `git` call, and re-checked registry-side against the node's actual role as the real enforcement point). A child's `repo`/`name` are never touched by this: `repo` is fixed at spawn time (inherited from its parent), and `name` is its given/task-derived label — a child's own worktree commonly lives in a sibling directory with its own git root, so self-reporting from cwd the way a workspace does would misname it after that worktree instead of the workspace it belongs to. Net effect: a workspace's tree label snaps from the launch directory to the real repo root's name (stable across further `cd`s within that repo, since `--show-toplevel` always resolves to the root) the moment the first status push — typically a harness's SessionStart-equivalent hook — fires from inside wherever the bare shell was `cd`'d into. It does **not** update live on a bare `cd` with no agent running yet, same "top up on next push, not on every keystroke" posture as branch — Overseer has no push signal for a shell doing nothing but changing directory.
...
| click-drag (over the pane) | Select text; releasing copies it to the system clipboard — mouse-native, no terminal-specific modifier key needed (see "Pane Text Selection" below) |
...
`crossterm`'s `EnableMouseCapture` (`tui.rs`, armed once at startup alongside the alternate screen and disarmed only at TUI exit) is a single, all-or-nothing command: it writes `CSI ?1000h ?1002h ?1003h ?1015h ?1006h` — click tracking, drag tracking, and *all* motion tracking, plus RXVT and SGR coordinate encoding — with no crossterm-level flag to arm a narrower subset (e.g. clicks only, no motion). Overseer needs that full mode for the entire session because wheel-scroll-forwarding (above) depends on it: a focused pane's own mouse-reporting TUI only receives wheel/drag reports at all when capture stays armed continuously, in every pane, focused or not. That's exactly the mode that makes most terminal emulators route every click-drag straight to Overseer instead of doing native OS-level text selection — the host terminal never even sees the gesture as its own.

Rather than push that gap onto the user, Overseer answers it the same way it already answers wheel-scrolling: by owning the raw mouse stream and implementing the behavior itself instead of relying on a terminal-native mechanic mouse capture makes unreachable. A left-button `Down` over the pane starts tracking a selection at that pane-local `(col, row)`; each `Drag` extends its cursor end (clamped to the pane's edges, same "clamp, don't lose the gesture" choice a scroll held past its bound gets); `Up` finalizes it. A real drag (cursor moved from the anchor) reverse-highlights the spanned cells — reading order, not a block/column selection — and copies the text out via an **OSC 52** clipboard-set escape sequence (`\x1b]52;c;<base64>\x07`), written straight to this process's own stdout the same way `notify::ring_bell` writes BEL. This is the same trick tmux, Neovim, and kitty's own copy path use to support real system-clipboard copy while still owning 100% of the mouse stream — terminals that don't recognize OSC 52 just ignore the unrecognized escape sequence, so it's always safe to send. A plain click with no movement clears any prior selection instead of copying a single cell (that gesture is the pre-existing click-to-focus/jump-in action, handled independently — see the keybinding table above). The pure range/extraction/encoding logic lives in `overseer_core::selection` (tested directly); `tui.rs`'s `start_pane_selection`/`update_pane_selection`/`finish_pane_selection` are the thin crossterm-facing controller on top, mirroring the wheel path's own `handle_mouse_wheel` split.

Scoped to the **pane only**, not the tree list — `Down`/`Drag`/`Up` were unclaimed before this (silently dropped; only wheel events and click-to-focus did anything), so claiming them for the pane is not a behavior change to anything that previously worked, and it doesn't forward into the focused inner PTY the way wheel events do (an inner mouse-reporting program's own click/drag handling, e.g. a TUI with its own mouse-driven UI, is a separate, not-yet-requested feature). A selection is tied to the agent whose grid it was drawn against and is dropped the instant the selected agent changes (same trigger that resets scroll-to-bottom), so a stale range can never paint over a different agent's pane.

**If your terminal doesn't honor OSC 52** (rare, but not universal — and some multiplexers gate it, e.g. tmux needs `set -g set-clipboard on`), or you'd simply rather use the terminal's own native drag-select: every terminal that supports mouse reporting also ships a modifier-key escape hatch — hold it while dragging and *the terminal* handles the selection itself instead of ever forwarding the drag to Overseer. The key isn't universal, so here's what actually works, terminal by terminal:

| Terminal | Hold while dragging, to select text natively |
|---|---|
| iTerm2 | `Option` |
| Apple Terminal.app | `Option` (or disable capture entirely via `View > Allow Mouse Reporting`) |
| Alacritty | `Shift` |
| kitty | `Shift` |
| WezTerm / **Kaku** | `Shift` (`bypass_mouse_reporting_modifiers`, configurable) |
| Ghostty | `Shift` (see `mouse-shift-capture` if an app needs to see the Shift state itself) |
| GNOME Terminal (VTE-based) | `Shift` (hardcoded — VTE exposes no setting to disable it) |
| Warp | `Shift` |

The macOS-native pair (iTerm2, Terminal.app) uses `Option`; nearly everything else follows xterm's original `Shift` convention. If your terminal isn't listed, its docs likely call this "bypass mouse reporting" or "shift-click override" — the mechanism is close to universal even where the exact key isn't.
...
| Clipboard copy | `base64` — encodes a pane text selection for the OSC 52 escape sequence (see "Pane Text Selection"); already resolved transitively via `alacritty_terminal`, promoted to a direct dependency rather than hand-rolled |
